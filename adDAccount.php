...
<html lang="en">
  ...
  <body>
    ...
    <nav class="Pnavbar mb-5">
      <ul>
        <li><a href="adTAccount.php">Teacher's Account</a></li>
        <li><a class="active" href="adDAccount.php">Driver's Account</a></li>
      </ul>
    </nav>
    ...
    <div class="body" style="max-width: 1600px; min-width: 1600px; flex-direction: row;">
        ...
        <div class="container my-4">
          <form action="" method="post">
            <div>
              ...
              <input type="text" name="ID" class="form-control" id="ID" min-length="8" maxlength="8" required value="145510" style="width: 43.5%;"/>
            </div>
            <div>
      ...
      <div class="Mcol">
        <div style="max-width: 900px;">
          <div class="container my-4">
            <h5 class="h5 mb-3">Driver's Done Counter Table:</h5>
      ...
      <div class="Rcol">
        <div class="container my-4">
          ...
          <?php
            ...
            $sql = "SELECT * FROM `dlogin` WHERE `loginIdD` = $ID";
            $result = mysqli_query($conn,$sql);
            $num = mysqli_num_rows($result);
            if($num > 0){

            // Total passenger
            $sql = "SELECT * FROM `bookmicro` WHERE `loginIdD` = $ID AND `date` BETWEEN '$FDofTmonth' AND '$LDofTmonth'";
            $result = mysqli_query($conn,$sql);
            $num = mysqli_num_rows($result);
            echo '<b>Total Passenger:<span style="color: red">'.$num.'</span></b><br>';

              // Total trip
              $sql = "SELECT * FROM `counter` WHERE `loginIdD` = $ID AND `date` BETWEEN '$FDofTmonth' AND '$LDofTmonth'";
              $result = mysqli_query($conn,$sql);
              $num = mysqli_num_rows($result);
              echo '<b>Total Trip:<span style="color: red">'.$num.'</span></b><br>';

              // Total Trip at Noon
              $sql = "SELECT * FROM `bookmicro` WHERE `loginIdD` = $ID AND `date` BETWEEN '$FDofTmonth' AND '$LDofTmonth' AND `time` = 'Noon' ";
              $result = mysqli_query($conn,$sql);
              $num = mysqli_num_rows($result);
              echo '<b>Total Trip at Noon:<span style="color: red">'.$num.'</span></b><br>';

              // Total Trip at Evening
              $sql = "SELECT * FROM `bookmicro` WHERE `loginIdD` = $ID AND `date` BETWEEN '$FDofTmonth' AND '$LDofTmonth' AND `time` = 'Evening' ";
              $result = mysqli_query($conn,$sql);
              $num = mysqli_num_rows($result);
